...
def MiscDevicePage(req):
    ...
    if req.method=='POST':
        ...
        new_block=MiscDevice.objects.create(asset_id=ai,device_name=name,desc=de,location_id=loc,serial_no=sn,model=m,make=mk,catagory=tp,price=pr,buy_date=bd,warranty=war,status=st,employee_id=emp)
        new_block.save()
        pdata=PC.objects.all()
        prdata=Printer.objects.all()
        edata=Employees.objects.all()
